// Code generated by qtc from "base.qtpl". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

//line internal/templates/layouts/base.qtpl:1
package layouts

//line internal/templates/layouts/base.qtpl:1
import "cms/internal/models"

//line internal/templates/layouts/base.qtpl:2
import "cms/internal/templates/components"

//line internal/templates/layouts/base.qtpl:4
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line internal/templates/layouts/base.qtpl:4
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line internal/templates/layouts/base.qtpl:5
// PageData is the interface required by the BaseLayout function.
type PageData = models.PageData

// <!-- BaseLayout defines the main HTML structure. -->

//line internal/templates/layouts/base.qtpl:10
func StreamBaseLayout(qw422016 *qt422016.Writer, data PageData, pageContent func() string) {
//line internal/templates/layouts/base.qtpl:10
	qw422016.N().S(`
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>`)
//line internal/templates/layouts/base.qtpl:16
	qw422016.E().S(data.Title())
//line internal/templates/layouts/base.qtpl:16
	qw422016.N().S(`</title>
    <meta name="description" content="`)
//line internal/templates/layouts/base.qtpl:17
	qw422016.E().S(data.Description())
//line internal/templates/layouts/base.qtpl:17
	qw422016.N().S(`">
    <link rel="icon" type="image/svg+xml" href="data:image/svg+xml;base64," />
    
    <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script>
    <style>
        /* Basic style to prevent flash of unstyled content with Alpine.js */
        [x-cloak] { display: none !important; }
    </style>
</head>
<body class="bg-gray-100 flex flex-col min-h-screen">
    <!-- Render the header component -->
    `)
//line internal/templates/layouts/base.qtpl:28
	qw422016.N().S(components.Header())
//line internal/templates/layouts/base.qtpl:28
	qw422016.N().S(`

    <main class="container mx-auto px-4 py-8 mt-16 flex-grow">
        <!-- Render the page-specific content passed as a function -->
        `)
//line internal/templates/layouts/base.qtpl:32
	qw422016.N().S(pageContent())
//line internal/templates/layouts/base.qtpl:32
	qw422016.N().S(`
    </main>

    <!-- Render the footer component -->
    `)
//line internal/templates/layouts/base.qtpl:36
	qw422016.N().S(components.Footer())
//line internal/templates/layouts/base.qtpl:36
	qw422016.N().S(`
</body>
</html>
`)
//line internal/templates/layouts/base.qtpl:39
}

//line internal/templates/layouts/base.qtpl:39
func WriteBaseLayout(qq422016 qtio422016.Writer, data PageData, pageContent func() string) {
//line internal/templates/layouts/base.qtpl:39
	qw422016 := qt422016.AcquireWriter(qq422016)
//line internal/templates/layouts/base.qtpl:39
	StreamBaseLayout(qw422016, data, pageContent)
//line internal/templates/layouts/base.qtpl:39
	qt422016.ReleaseWriter(qw422016)
//line internal/templates/layouts/base.qtpl:39
}

//line internal/templates/layouts/base.qtpl:39
func BaseLayout(data PageData, pageContent func() string) string {
//line internal/templates/layouts/base.qtpl:39
	qb422016 := qt422016.AcquireByteBuffer()
//line internal/templates/layouts/base.qtpl:39
	WriteBaseLayout(qb422016, data, pageContent)
//line internal/templates/layouts/base.qtpl:39
	qs422016 := string(qb422016.B)
//line internal/templates/layouts/base.qtpl:39
	qt422016.ReleaseByteBuffer(qb422016)
//line internal/templates/layouts/base.qtpl:39
	return qs422016
//line internal/templates/layouts/base.qtpl:39
}
